<template>
  <r-popup v-model="visible" position="bottom" class="rkhy-datetime">
    <r-picker
      :slots="dateSlots"
      @change="onChange"
      :visible-item-count="visibleItemCount"
      class="rkhy-datetime-picker"
      ref="picker"
      show-toolbar>
      <span class="rkhy-datetime-action rkhy-datetime-cancel" @click="visible = false;$emit('cancel')">{{ cancelText }}</span>
      <span class="rkhy-datetime-action rkhy-datetime-confirm" @click="confirm">{{ confirmText }}</span>
    </r-picker>
  </r-popup>
</template>
...
<script>
    ...
    /**
     * @param {String} cancelText - 右上角关闭的文字
     * @param {String} confirmText - 左上角确定的文字
     * @param {String} type - 日期选择的类型有三种datetime,date,time
     * @param {Date} startDate - 日期的最小可选值
     * @param {Date} endDate - 日期的最大可选值
     * @param {Number} startHour - 小时的最小可选值（0-23）
     * @param {Number} endHour - 小时的最大可选值（0-23）
     * @param {String} yearFormat -  年份模板"{value} 年"
     * @param {String} monthFormat -  月份模板"{value} 月"
     * @param {String} dateFormat -  日期模板"{value} 天"
     * @param {String} hourFormat -  小时模板"{value} 时"
     * @param {String} minuteFormat -  分钟模板"{value} 分"
     * @param {Number} visibleItemCount -  弹框里能展示出的条数
     * @param {Number} value -  选中的值会发送到父组件v-model的值里
     *
     *可用事件
     *confirm {function} 点击确定时候触发，参数是当前选中的值
     * handleValueChange {function} 选择滑动的时候触发，参数是当前选中的值
     *
     *
     *该组件需要触发组件内部的open方法弹出 例：this.$refs.picker.open()
     **/



  import RPicker from '~/picker/index.js';
  import RPopup from '~/popup/index.js';
